...
const presets: Record<PresetKey, { label: string; duration: number; description: string }> = {
  conservative: {
    label: "Conservative",
    duration: 12.8,
    description: "Long preview, low jerk limits, low risk.",
  },
  aggressive: {
    label: "Aggressive",
    duration: 10.6,
    description: "Short lookahead, late braking, hungry throttle.",
  },
}
...
export default function ChallengeGhostPlayground() {
  ...
  const lapSeconds = useMemo(() => {
    ...
    return clamp(11 * riskBoost * previewPenalty * gainBoost * kpBoost * jerkPenalty, 9.2, 13.4)
  }, [state])
  ...
  const gapSeconds = useMemo(() => ghost.duration - lapSeconds, [ghost.duration, lapSeconds])
  const gapLabel = gapSeconds > 0 ? "You lead this ghost" : "Ghost is ahead"
  const normalizedGap = clamp(gapSeconds / 4, -1, 1)
  ...
  return (
    <div className="mb-10 overflow-hidden rounded-3xl border bg-gradient-to-br from-muted/50 via-background to-primary/5 p-6 shadow-sm ring-1 ring-border/60 dark:to-sky-950/10">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Challenge: tame the oval + ghost race
          </div>
          <div className="text-sm text-muted-foreground">
            Tune the baseline hyper-parameters, then outrun the baseline presets.
          </div>
        </div>
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="relative flex-1 overflow-hidden rounded-2xl border bg-gradient-to-br from-white/70 via-background to-muted/40 p-3 shadow-inner backdrop-blur dark:from-neutral-900/50 dark:via-background dark:to-neutral-900/40">
          ...
          <svg viewBox="0 0 210 150" className="relative h-[230px] w-full" role="presentation">
            <defs>
              <linearGradient id="challengeTrackGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <motion.path
              d={pathD}
              fill="none"
              stroke="url(#challengeTrackGradient)"
              strokeWidth={4}
              strokeLinecap="round"
          ...
          <div className="pointer-events-none absolute right-3 top-3 flex flex-col gap-2 rounded-xl border border-white/30 bg-white/85 px-3 py-2 text-[11px] shadow-sm backdrop-blur dark:border-white/10 dark:bg-neutral-900/85">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Ghost preset</div>
            <div className="text-sm font-semibold text-foreground">{ghost.label}</div>
            <div className="text-[11px] text-muted-foreground">{ghost.description}</div>
          </div>
        ...
        <div className="w-full space-y-4 lg:w-80">
          <div className="rounded-2xl border bg-white/70 p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900/70">
            ...
            <div className="mt-3 flex items-baseline gap-2">
              <motion.div
                ...
                className={cn(
                  ...
                  gapSeconds > 0 ? "text-emerald-600 dark:text-emerald-300" : "text-amber-600 dark:text-amber-300"
                )}
              >
                {Math.abs(gapSeconds).toFixed(2)}s
              </motion.div>
              <div className="text-sm text-muted-foreground">{gapLabel}</div>
            </div>
            <div className="relative mt-4 h-2 rounded-full bg-muted">
              ...
              <motion.div
                className={cn(
                  ...
                  gapSeconds > 0 ? "bg-emerald-500" : "bg-amber-500"
                )}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div>
                <div className="text-xs uppercase tracking-wide text-muted-foreground">Your lap</div>
                <div className="font-semibold text-foreground">{lapSeconds.toFixed(2)} s</div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-wide text-muted-foreground">Ghost lap</div>
                <div className="font-semibold text-foreground">{ghost.duration.toFixed(2)} s</div>
              </div>
            </div>
          </div>
